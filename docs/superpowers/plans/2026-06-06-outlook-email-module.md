# Outlook Email Module Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build and wire up a hardcoded v2 Circuit module that teaches Year 7–8 students to write emails in Microsoft Outlook, using the existing V2LessonPlayer and all block types already in the registry.

**Architecture:** The module is authored as a TypeScript data file at `src/data/v2/outlook-email-module.ts` using inline block objects with stable string IDs (not factory random UUIDs — hardcoded modules need consistent IDs across page loads for progress tracking). A new `src/data/v2/module-registry-v2.ts` exports a lookup function. `ModulePlayer` gains a second fallback: DB → v2 hardcoded → legacy hardcoded → not found.

**Tech Stack:** React 18, TypeScript, Vite, Tailwind CSS, shadcn/ui. No new dependencies, no Supabase migration needed.

**Spec:** `docs/superpowers/specs/2026-06-06-outlook-email-module-design.md`
...
| File | Action |
|---|---|
| `src/data/v2/outlook-email-module.ts` | Create — full module data (3 lessons, 17 steps) |
| `src/data/v2/module-registry-v2.ts` | Create — lookup function for hardcoded v2 modules |
| `src/pages/ModulePlayer.tsx` | Modify — add v2 hardcoded fallback to load chain |

No new block renderers needed — `text`, `callout`, `word-match`, `true-false`, `fill-blank`, `quiz`, `sequence`, `flashcard` are all already registered in `BlockRegistry.tsx`.

---

## Task 1: Create the module data file

**Files:**
- Create: `src/data/v2/outlook-email-module.ts`

> **Why explicit IDs?** Factory functions in `src/lib/blocks/factories.ts` call `crypto.randomUUID()` on every import. For a hardcoded module, block/step IDs must be stable across page reloads so that `block_responses` stored in Supabase match correctly on return visits. Use explicit descriptive string IDs throughout.

- [ ] **Step 1: Create the directory and file**

Create `src/data/v2/outlook-email-module.ts` with the following content in full:
...
Expected: no errors. If you see errors about `StepBlock` union not including a type, check that all `type` string literals match exactly the types defined in `src/types/module-v2.ts` (e.g. `'word-match'`, `'true-false'`, `'fill-blank'`, `'sequence'`, `'flashcard'`, `'quiz'`).

- [ ] **Step 3: Commit**

```bash
git add src/data/v2/outlook-email-module.ts
git commit -m "feat(modules): add outlook-email v2 module (3 lessons, Year 7-8)"
```

---

## Task 2: Create v2 registry and wire into ModulePlayer

**Files:**
- Create: `src/data/v2/module-registry-v2.ts`
- Modify: `src/pages/ModulePlayer.tsx`

- [ ] **Step 1: Create the v2 registry**
...
- [ ] **Step 2: Add the v2 import to ModulePlayer**

Open `src/pages/ModulePlayer.tsx`. Add this import alongside the existing registry import:

```typescript
import { getV2ModuleById } from '@/data/v2/module-registry-v2';
```

The existing import to keep:
```typescript
import { getModuleById } from '@/data/module-registry';
```

- [ ] **Step 3: Add v2Module state to ModulePlayer**

In `src/pages/ModulePlayer.tsx`, find this block:
...
Add the new state directly after the `legacyModule` line:
...
- [ ] **Step 4: Update the load useEffect**

Find and replace the existing `useEffect` that loads the module:
...
- [ ] **Step 5: Update isV2, activeV2Module, and displayModule**
...
- [ ] **Step 6: Update the V2 lesson lookup**

Find and replace the v2 lesson lookup inside the active lesson block:
...
Expected: no errors. If you see `activeV2Module` referenced before declaration, check that the `activeV2Module` const is defined before `displayModule` (they go on adjacent lines — order matters).

- [ ] **Step 8: Verify the module loads in the browser**
...
Navigate to the student dashboard and look for the "Writing Emails on Microsoft Outlook" module card, or go directly to `/module/outlook-email`. Verify:
1. The module landing page loads with title and 3 lessons listed
2. Clicking Lesson 1 opens the V2LessonPlayer (not the legacy LessonPlayer)
3. Step 1 shows the "What Is Email?" instruction text and why-it-matters callout
4. The Continue button is enabled (instruction step, no interactive blocks)
5. Step 3 shows the word-match block and the Continue button is disabled until all 5 pairs are matched
6. Step 4 shows 3 true-false blocks — Continue stays disabled until all 3 are answered

- [ ] **Step 9: Commit**

```bash
git add src/data/v2/module-registry-v2.ts src/pages/ModulePlayer.tsx
git commit -m "feat(modules): wire v2 hardcoded registry into ModulePlayer load chain"
```
...
**Spec coverage:**
- ✅ Module ID `outlook-email`, title, 35 min, Year 7-8 — Task 1 Step 1
- ✅ Lesson 1 (5 steps): instruction ×2, word-match, true-false ×3, sequence — Task 1 Step 1
- ✅ Lesson 2 (6 steps): instruction ×2, word-match, fill-blank, true-false ×3, quiz — Task 1 Step 1
- ✅ Lesson 3 (6 steps): instruction ×2, true-false ×3, sequence, fill-blank, flashcard — Task 1 Step 1
- ✅ v2 hardcoded registry — Task 2 Step 1
- ✅ ModulePlayer load chain: DB → v2 → legacy — Task 2 Steps 2–6
- ✅ Blank 4 (student's own name) known limitation documented inline in code — Task 1 Step 1

**Placeholder scan:** No TBDs, no vague steps, all code is complete and explicit.

**Type consistency:**
- `outlookEmailModule` exported from `outlook-email-module.ts`, imported in `module-registry-v2.ts` ✓
- `getV2ModuleById` defined in `module-registry-v2.ts`, imported in `ModulePlayer.tsx` ✓
- `activeV2Module` defined before `displayModule` in the same block ✓
- All block `type` literals match the `StepBlock` union in `module-v2.ts` ✓
- `scoring.hints: []` satisfies `hints: string[]` (required, not optional) ✓
